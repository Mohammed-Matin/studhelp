    ...
    if (!club || !isManager(club.userRole)) {
        return (
            <div className="p-6 max-w-2xl mx-auto text-center min-h-screen bg-[#07070f]">
                <p className="text-red-400 mb-4">{error || 'You do not have permission to create events for this club'}</p>
                <Link to={`/clubs/${clubId}`} className="text-cyan-400 hover:underline">← Back to Club</Link>
            </div>
        );
    }
        ...
        <div className="min-h-screen bg-[#07070f]">
            <div className="relative border-b border-white/5 py-12 px-6 overflow-hidden">
                ...
                <div className="relative max-w-3xl mx-auto">
                    <Link to={`/clubs/${clubId}`} className="text-cyan-400 hover:text-cyan-300 text-sm mb-4 inline-block">
                        ← Back to {club.name}
                    </Link>
                    <p className="text-xs tracking-[0.2em] uppercase text-purple-400 mb-2">Host an Event</p>
                    <h1 className="font-display text-3xl md:text-4xl font-bold text-white">Create <span className="text-gradient">Event</span></h1>
                </div>
            </div>
                ...
                <form onSubmit={handleSubmit} className="p-8 space-y-6">
                    ...
                    {clashes.length > 0 && (
                        <div className="bg-amber-500/10 border border-amber-500/30 rounded-lg p-4 text-sm">
                            <p className="font-medium text-amber-300">Schedule clash detected</p>
                    ...
                    <section>
                        <h2 className="text-sm font-semibold text-cyan-400 uppercase tracking-wider mb-4">Basic Details</h2>
                            ...
                            <div>
                                ...
                                <div className="flex gap-2 mt-2 flex-wrap">
                                    {SUGGESTED_BANNERS.map((url) => (
                                        <button key={url} type="button" onClick={() => setForm({ ...form, banner_url: url })}
                                                className="w-20 h-12 rounded-lg overflow-hidden border border-white/10 hover:border-purple-500/50 transition">
                                            <img src={url} alt="" className="w-full h-full object-cover" />
                                        </button>
                                    ))}
                                </div>
                                <p className="text-xs text-slate-500 mt-1">Saved to database when event is created</p>
                            </div>
                        </div>
                    </section>

                    <section>
                        <h2 className="text-sm font-semibold text-cyan-400 uppercase tracking-wider mb-4">Schedule</h2>
                    ...
                    <section>
                        <h2 className="text-sm font-semibold text-cyan-400 uppercase tracking-wider mb-4">Participation</h2>
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            ...
                            <div>
                                <label className="block text-sm font-medium text-slate-300 mb-1">Max Teams</label>
                                <input
                                    type="number"
                                    min="1"
                    ...
                    <div className="flex gap-3 justify-end pt-4 border-t border-white/10">
                        <Link to={`/clubs/${clubId}`} className="btn-ghost">Cancel</Link>
