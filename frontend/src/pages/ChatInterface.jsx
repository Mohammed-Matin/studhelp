...
const ChatInterface = () => {
    ...
    const loadMessages = useCallback(async () => {
        ...
        try {
            if (activeChat.type === 'dm') {
                ...
                const res = await axiosInstance.get(`/messages/group/${activeChat.type}/${activeChat.id}`);
                setMessages(res.data);
            }
        } catch (err) {
        ...
        socketRef.current.on('receive_message', (messageData) => {
            ...
            if (chat) {
                const isRelevant = chat.type === 'dm'
                    ? (messageData.sender_id === chat.id || messageData.receiver_id === chat.id)
                    : (messageData.group_type === chat.type && messageData.receiver_id === chat.id);
                if (isRelevant) {
    ...
    const selectConversation = (conv) => {
        startDM({ id: conv.user_id, full_name: conv.full_name, username: conv.username });
    };
            ...
            <div className="w-80 bg-white border-r flex flex-col">
                ...
                <div className="flex-1 overflow-y-auto">
                    ...
                    {conversations.length === 0 ? (
                        <div className="p-4 text-center text-gray-400 text-sm">No conversations yet. Search for someone to message.</div>
                    ) : (
                        conversations.map((conv) => (
                            <button key={conv.user_id} onClick={() => selectConversation(conv)}
                                    className={`w-full px-4 py-3 text-left hover:bg-gray-50 border-b flex items-center gap-3 transition ${
                                        activeChat?.id === conv.user_id && activeChat?.type === 'dm' ? 'bg-blue-50' : ''
                                    }`}>
                                ...
                                <div className="flex-1 min-w-0">
                                    <p className="font-medium text-sm truncate">{conv.full_name || conv.username}</p>
                ...
                {activeChat ? (
                        ...
                        <div className="bg-white border-b px-6 py-4 flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-gray-500">
                                    {activeChat.type === 'dm' ? `@${activeChat.username}` : activeChat.type}
                                </p>
                            </div>
                        </div>
                        ...
                        <div className="flex-1 overflow-y-auto p-6 space-y-4">
                            {messages.length === 0 ? (
                                ...
                                messages.map((msg, idx) => {
                                    ...
                                    const displayName = isAnonymous ? 'Anonymous' : (msg.sender_name || 'Unknown');
                                    const timeStr = msg.timestamp
                                        ? new Date(msg.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
                                        : '';
                                    return (
                                        <div key={msg.id || idx} className={`flex ${isMe(msg.sender_id) ? 'justify-end' : 'justify-start'}`}>
                                            <div className={`max-w-[70%] ${isMe(msg.sender_id)
                                                ? 'bg-blue-600 text-white rounded-2xl rounded-br-sm'
                                                : 'bg-white border shadow-sm rounded-2xl rounded-bl-sm'
                                            } px-4 py-2.5`}>
                                                {!isMe(msg.sender_id) && activeChat.type !== 'dm' && (
                                                    <p className={`text-xs font-medium mb-1 ${isAnonymous ? 'text-gray-400 italic' : 'text-blue-600'}`}>
                                                        {isAnonymous ? '🕵️ Anonymous' : displayName}
                                                    </p>
                                                )}
                                                {isMe(msg.sender_id) && isAnonymous && (
                                                    <p className="text-xs font-medium mb-1 text-blue-200 italic">Anonymous</p>
                                                )}
                    ...
                    <div className="flex-1 flex items-center justify-center text-gray-400">
                        <div className="text-center">
                            ...
                            <p className="text-sm mt-1">Choose a chat from the sidebar or search for someone</p>
                        </div>
                    </div>
                )}
            </div>
